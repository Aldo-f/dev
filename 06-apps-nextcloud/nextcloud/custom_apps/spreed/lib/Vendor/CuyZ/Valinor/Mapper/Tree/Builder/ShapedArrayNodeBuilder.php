...
use OCA\Talk\Vendor\CuyZ\Valinor\Mapper\Tree\Exception\UnexpectedKeyInSource;
...
final class ShapedArrayNodeBuilder implements NodeBuilder
{
    public function build(Shell $shell): Node
    {
        ...
        foreach ($type->elements as $key => $element) {
            $child = $shell->child((string) $key, $element->type())->withAttributes($element->attributes());
        ...
        if ($type->isUnsealed) {
            $unsealedNode = $shell->withType($type->unsealedType())->withValue($value)->build();
