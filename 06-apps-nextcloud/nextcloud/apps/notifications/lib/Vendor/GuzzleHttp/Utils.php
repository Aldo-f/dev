...
use OCA\Notifications\Vendor\GuzzleHttp\Handler\CurlHandler;
use OCA\Notifications\Vendor\GuzzleHttp\Handler\CurlMultiHandler;
...
use OCA\Notifications\Vendor\GuzzleHttp\Handler\Proxy;
use OCA\Notifications\Vendor\GuzzleHttp\Handler\StreamHandler;
...
final class Utils
{
    ...
    {
        \trigger_deprecation('guzzlehttp/guzzle', '7.12', '%s() is deprecated and will be removed in 8.0. Use get_debug_type() instead.', __METHOD__);
    ...
    private static function parseNoProxyRule(string $area): ?array
    {
        ...
        if ($host === '*') {
            ...
            return ['type' => 'wildcard', 'value' => '*', 'port' => $port, 'matchesRoot' => true];
        }
    ...
    private static function parseNoProxyHost(string $host, ?int $port, bool $matchesRoot): ?array
    {
        ...
        return ['type' => 'domain', 'value' => Psr7\Utils::asciiToLower($host), 'port' => $port, 'matchesRoot' => $matchesRoot];
    }
    /**
    ...
    public static function normalizeIdnConversionOption($value): ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            \trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', \get_debug_type($value));
            return (int) $value;
        }
