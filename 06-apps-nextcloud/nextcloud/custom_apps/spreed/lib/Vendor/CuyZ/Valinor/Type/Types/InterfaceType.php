...
final class InterfaceType implements ObjectType, ObjectWithGenericType
{
    ...
    {
        return new self($this->interfaceName, array_map($callback, $this->generics));
    }
    ...
    {
        return empty($this->generics) ? $this->interfaceName : $this->interfaceName . '<' . implode(', ', array_map(static fn(Type $type) => $type->toString(), $this->generics)) . '>';
    }
}