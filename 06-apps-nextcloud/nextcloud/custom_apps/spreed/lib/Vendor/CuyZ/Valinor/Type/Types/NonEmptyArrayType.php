...
final class NonEmptyArrayType implements CompositeTraversableType, DumpableType
{
    ...
    public function __construct(private ArrayKeyType $keyType, private Type $subType)
    {
    }
    ...
    public function accepts(mixed $value): bool
    {
        ...
        return Polyfill::array_all($value, fn(mixed $item, mixed $key) => $this->keyType->accepts($key) && $this->subType->accepts($item));
    }
    public function compiledAccept(ComplianceNode $node): ComplianceNode
    {
        $condition = Node::logicalAnd($node->different(Node::value([])), Node::functionCall('is_array', [$node]));
        if ($this === self::native()) {
            return $condition;
        }
    ...
    public function matches(Type $other): bool
    {
        ...
        return $this->keyType->matches($other->keyType()) && $this->subType->matches($other->subType());
    }
    ...
    {
        return new self($callback($this->keyType), $callback($this->subType));
    }
    ...
    public function toString(): string
    {
        ...
        return $this->keyType === ArrayKeyType::default() ? "non-empty-array<{$this->subType->toString()}>" : "non-empty-array<{$this->keyType->toString()}, {$this->subType->toString()}>";
    }
}