...
final class NonEmptyListType implements CompositeTraversableType, DumpableType
{
    ...
    {
        $condition = Node::logicalAnd($node->different(Node::value([])), Node::functionCall('is_array', [$node]), Node::functionCall('array_is_list', [$node]));
        if ($this === self::native()) {
            return $condition;
        }
        return $condition->and(Node::functionCall(function_exists('array_all') ? 'array_all' : Polyfill::class . '::array_all', [$node, Node::shortClosure($this->subType->compiledAccept(Node::variable('item'))->wrap())->witParameters(Node::parameterDeclaration('item', 'mixed'))]));
    }
