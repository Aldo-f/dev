...
namespace OCA\Notifications\Vendor\GuzzleHttp\Handler;

use OCA\Notifications\Vendor\GuzzleHttp\Exception\ConnectException;
use OCA\Notifications\Vendor\GuzzleHttp\Exception\RequestException;
use OCA\Notifications\Vendor\GuzzleHttp\Multiplexing;
use OCA\Notifications\Vendor\GuzzleHttp\Promise as P;
use OCA\Notifications\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use OCA\Notifications\Vendor\GuzzleHttp\Promise\PromiseInterface;
use OCA\Notifications\Vendor\GuzzleHttp\Psr7;
use OCA\Notifications\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use OCA\Notifications\Vendor\GuzzleHttp\Psr7\Uri;
use OCA\Notifications\Vendor\GuzzleHttp\TransferStats;
use OCA\Notifications\Vendor\GuzzleHttp\TransportSharing;
use OCA\Notifications\Vendor\GuzzleHttp\Utils;
use OCA\Notifications\Vendor\Psr\Http\Message\RequestInterface;
use OCA\Notifications\Vendor\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
     ...
     */
    private const STRINGABLE_PROXY_CREDENTIAL_OPTIONS = ['CURLOPT_PROXYUSERPWD', 'CURLOPT_PROXYUSERNAME', 'CURLOPT_PROXYPASSWORD', 'CURLOPT_PROXY_SSLCERT', 'CURLOPT_PROXY_SSLKEY', 'CURLOPT_PROXY_KEYPASSWD', 'CURLOPT_PROXY_TLSAUTH_USERNAME', 'CURLOPT_PROXY_TLSAUTH_PASSWORD'];
    /**
     * @deprecated
     */
    ...
    public function create(RequestInterface $request, array $options): EasyHandle
    {
        ...
        if ($requiredMultiplex && isset($options['curl']) && \is_array($options['curl'])) {
            $requiredModeConflicts = [\CURLOPT_HTTP_VERSION => ['CURLOPT_HTTP_VERSION', 'the request protocol version'], \CURLOPT_URL => ['CURLOPT_URL', 'the request URI'], \CURLOPT_FOLLOWLOCATION => ['CURLOPT_FOLLOWLOCATION', 'the "allow_redirects" request option']];
        ...
        $rawHttpHeadersReplaceManaged = isset($options['curl']) && \is_array($options['curl']) && \array_key_exists(\CURLOPT_HTTPHEADER, $options['curl']);
        $easy = new EasyHandle();
    ...
    private function rejectRequestLevelShareConflict(array $options): void
    {
        ...
        if (!\defined('CURLOPT_SHARE') || !isset($options['curl']) || !\is_array($options['curl']) || !\array_key_exists((int) \constant('CURLOPT_SHARE'), $options['curl'])) {
            return;
        }
    ...
    private static function assertRequiredMultiplexRouteDirect(EasyHandle $easy, array $conf): void
    {
        ...
        if (self::getEffectiveProxy($conf) !== null || \defined('CURLOPT_PRE_PROXY') && ($conf[(int) \constant('CURLOPT_PRE_PROXY')] ?? '') !== '') {
    ...
    private static function resolveProxy(RequestInterface $request, array $options): array
    {
        ...
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    if (isset($options['proxy']['no']) && Utils::isUriInNoProxy($request->getUri(), $options['proxy']['no'])) {
                        $proxyConf = '';
                        $noProxyConf = '*';
                    } else {
        ...
        if ($proxyConf === null) {
            ...
            if ($proxyConf === null) {
                $proxyConf = '';
            } elseif (($noProxy = ProxyEnvironment::getNoProxy()) !== null && Utils::isUriInNoProxy($request->getUri(), ProxyEnvironment::splitNoProxy($noProxy))) {
                $proxyConf = '';
                $noProxyConf = '*';
            }
        }
    ...
    {
        return \defined('CURLOPT_SHARE') && isset($options['curl']) && \is_array($options['curl']) && \array_key_exists((int) \constant('CURLOPT_SHARE'), $options['curl']);
    }
    /**
     * @param array<int|string, mixed> $conf
     */
    private static function hasAuthenticatedHttpProxyState(string $proxy, array $conf): bool
    {
        ...
        if (\is_array($proxyParts) && (\array_key_exists('user', $proxyParts) || \array_key_exists('pass', $proxyParts))) {
            return true;
        }
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options): void
    {
        ...
        if (\defined('CURLOPT_PROXYHEADER') && \array_key_exists((int) \constant('CURLOPT_PROXYHEADER'), $options['curl']) && !CurlVersion::supportsProxyHeaderSeparation()) {
            \trigger_deprecation('guzzlehttp/guzzle', '7.15', \sprintf('Passing %s in the "curl" request option on a build without proxy header separation support is deprecated; guzzlehttp/guzzle 8.0 will reject this configuration because proxy headers require libcurl 7.37.0 or newer built with proxy header separation support.', self::formatCurlOption((int) \constant('CURLOPT_PROXYHEADER'))));
        }
        $supportedOptions = self::supportedCurlOptions();
        $conflictingOptions = self::conflictingCurlOptions();
        foreach ($options['curl'] as $option => $_) {
            if (!\is_int($option) || \array_key_exists($option, $supportedOptions) || \array_key_exists($option, $conflictingOptions)) {
                continue;
            }
            \trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    ...
    public function release(EasyHandle $easy): void
    {
        ...
        unset($easy->handle);
        if (\count($this->handles) >= $this->maxHandles || $easy->proxyTunnelSignature !== null && $easy->proxyTunnelSignature !== $this->proxyTunnelOwner) {
    ...
    public static function finish(callable $handler, EasyHandle $easy, CurlFactoryInterface $factory): PromiseInterface
    {
        ...
        if (isset($easy->options['on_trailers'])) {
            try {
                ...
                return P\Create::rejectionFor(new RequestException('An error was encountered during the on_trailers event', $easy->request, $easy->response, $e));
            }
        }
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        $message = \sprintf('cURL error %s: %s (%s)', $ctx['errno'], $sanitizedError, 'see https://curl.se/libcurl/c/libcurl-errors.html');
        if ('' !== $sanitizedError) {
    ...
    private function isolateOpaqueShareAnonymousProxyTunnel(RequestInterface $request, array &$conf): void
    {
        ...
        $proxy = self::getEffectiveProxy($conf);
        if ($proxy === null || !self::usesProxyTunnel($request, $conf) || !self::isHttpProxyForConnectionReuse($proxy, $conf) || self::hasAuthenticatedHttpProxyState($proxy, $conf)) {
            return;
        }
    ...
    private static function requiresFreshConnectionForAuthenticatedProxy(RequestInterface $request, string $proxy, array $conf): bool
    {
        ...
        if (!CurlVersion::supportsProxyTlsCredentialAwareConnectionReuse() && self::hasCurlProxyTlsCredentials($conf)) {
            return true;
        }
        if (CurlVersion::supportsProxyCredentialAwareConnectionReuse()) {
            return false;
        }
        return \array_key_exists('user', $proxyParts) || \array_key_exists('pass', $proxyParts) || self::hasCurlProxyCredentials($conf);
    }
    /**
     * @param array<int|string, mixed> $conf
     */
    ...
    private static function usesProxyTunnel(RequestInterface $request, array $conf): bool
    {
        ...
        return \defined('CURLOPT_HTTPPROXYTUNNEL') && \array_key_exists((int) \constant('CURLOPT_HTTPPROXYTUNNEL'), $conf) && (bool) $conf[(int) \constant('CURLOPT_HTTPPROXYTUNNEL')];
    }
    /**
     * @param array<int|string, mixed> $conf
     */
    private static function hasCurlConnectTo(array $conf): bool
    {
        ...
        $value = $conf[$option];
        return \is_array($value) ? $value !== [] : $value !== null && $value !== false && $value !== '';
    }
    /**
     * @param array<int|string, mixed> $conf
     */
    ...
    private static function socksProxySignature(string $proxy, array $conf): ?string
    {
        ...
        foreach (['CURLOPT_PROXYUSERPWD', 'CURLOPT_PROXYUSERNAME', 'CURLOPT_PROXYPASSWORD', 'CURLOPT_PROXYTYPE'] as $name) {
            $credentialState[$name] = \defined($name) ? $conf[(int) \constant($name)] ?? null : null;
        }
    ...
    private static function isSocksProxyType($proxyType): bool
    {
        ...
        foreach (['CURLPROXY_SOCKS4' => 4, 'CURLPROXY_SOCKS5' => 5, 'CURLPROXY_SOCKS4A' => 6, 'CURLPROXY_SOCKS5_HOSTNAME' => 7] as $name => $fallback) {
    ...
    {
        foreach (['CURLOPT_PROXY_SSLCERT', 'CURLOPT_PROXY_SSLCERT_BLOB', 'CURLOPT_PROXY_TLSAUTH_USERNAME', 'CURLOPT_PROXY_TLSAUTH_PASSWORD'] as $option) {
            if (\defined($option) && \array_key_exists((int) \constant($option), $conf)) {
                return true;
            }
        }
    ...
    private static function managedProxyAuthorizationHeaderLines(RequestInterface $request): array
    {
        ...
        foreach ($request->getHeader('Proxy-Authorization') as $value) {
            $headers[] = $value === '' ? 'Proxy-Authorization;' : 'Proxy-Authorization: ' . $value;
        }
        return $headers;
    }
    /**
    ...
    {
        return \defined('CURLOPT_PROXYHEADER') && \array_key_exists((int) \constant('CURLOPT_PROXYHEADER'), $conf);
    }
    ...
    private static function proxyTunnelSignature(RequestInterface $request, array $conf): ?string
    {
        ...
        $credentialState = [];
        foreach (['CURLOPT_PROXYUSERPWD', 'CURLOPT_PROXYUSERNAME', 'CURLOPT_PROXYPASSWORD', 'CURLOPT_PROXYTYPE', 'CURLOPT_PROXY_SSLCERT', 'CURLOPT_PROXY_SSLCERT_BLOB', 'CURLOPT_PROXY_SSLKEY', 'CURLOPT_PROXY_KEYPASSWD', 'CURLOPT_PROXY_TLSAUTH_USERNAME', 'CURLOPT_PROXY_TLSAUTH_PASSWORD', 'CURLOPT_PROXY_SSLVERSION'] as $name) {
            $credentialState[$name] = \defined($name) ? $conf[(int) \constant($name)] ?? null : null;
        }
    ...
    private function applyTlsVersionRange(EasyHandle $easy, array &$conf): void
    {
        ...
        if ($isHttp2 && $cryptoMethodMax !== null && TlsVersion::ordinal('crypto_method_max', $cryptoMethodMax) < 12) {
            throw new \InvalidArgumentException('Invalid crypto_method_max request option: HTTP/2 requires TLS 1.2 or higher');
        }
        ...
        $sslVersion = $cryptoMethod === null ? \CURL_SSLVERSION_DEFAULT : self::curlMinSslVersion($cryptoMethod);
    ...
    private function createHeaderFn(EasyHandle $easy): callable
    {
        ...
        return static function ($ch, $h) use ($onHeaders, $easy, &$startingResponse, &$collectingTrailers, $retainTrailers) {
            $value = \trim($h, " \n\r\t\x00\v");
            if ($h === "\r\n" || $h === "\n" || $h === "\r" || $h === '') {
