...
{
    public function __construct(public Shell $shell, private string|null $singleArgumentName = null)
    {
    }
    public static function forInterface(Shell $shell, Arguments $arguments): self
    {
        ...
        if (count($arguments) === 0) {
            return new self($shell->withValue([])->withType($arguments->toShapedArray()));
        }
    ...
    public static function forClass(Shell $shell, Arguments $arguments): self
    {
        ...
        if ($type instanceof UnionType) {
            $subTypes = $type->types();
            $filtered = array_filter($subTypes, static fn(Type $subType) => !$subType instanceof ObjectType || $subType->className() !== $shell->type->className());
            if ($filtered !== $subTypes) {
                $type = UnionType::from(...$filtered);
            }
        }
        return new self(shell: $shell->withType($type)->withAttributes($attributes), singleArgumentName: $argument->name());
    }
    /**
