...
use OCA\Talk\Vendor\CuyZ\Valinor\Mapper\AsConverter;
...
final class ReflectionAttributesRepository implements AttributesRepository
{
    ...
    public function for(Reflector $reflection): array
    {
        ...
        foreach ($reflection->getAttributes() as $key => $attribute) {
            ...
            $attributes[] = new AttributeDefinition($this->classDefinitionRepository->for(new NativeClassType($attribute->getName())), $arguments, match ($reflection::class) {
                ...
                ReflectionParameter::class => $reflection->getDeclaringFunction()->isClosure() ? ['closureParameter', $reflection->getPosition()] : ['methodParameter', $reflection->getDeclaringClass()->name, $reflection->getDeclaringFunction()->name, $reflection->getPosition()],
                default => ['closure'],
            }, $key);
        }
        return $attributes;
    ...
    private function attributeIsAllowed(ReflectionAttribute $attribute): bool
    {
        ...
        return Reflection::class($attribute->getName())->getAttributes(AsConverter::class) !== [] || Reflection::class($attribute->getName())->getAttributes(AsTransformer::class) !== [];
    }
    /**
     * @param ReflectionAttribute<object> $attribute
     */
