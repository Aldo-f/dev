...
final class IterableType implements CompositeTraversableType, DumpableType
{
    ...
    {
        $condition = Node::logicalAnd(Node::functionCall('is_iterable', [$node]), Node::negate($node->instanceOf(Generator::class)));
        if ($this === self::native()) {
            return $condition;
        }
    ...
    public function matches(Type $other): bool
    {
        ...
        return $other instanceof self && $this->keyType->matches($other->keyType()) && $this->subType->matches($other->subType());
    }
    ...
    public function toString(): string
    {
        ...
        return $this->keyType === ArrayKeyType::default() ? "iterable<{$this->subType->toString()}>" : "iterable<{$this->keyType->toString()}, {$this->subType->toString()}>";
    }
}