...
```bash
# bootstrap/update + deploy: single entrypoint at the monorepo root
cd ~/dev && ./install.sh
...
| Script | What it does |
|------- | ------------ |
| `~/dev/install.sh` | **The** installer (monorepo root). Syncs `~/dev` to `origin/main` safely (dirty tracked files are auto-stashed and popped back; `INSTALL_SKIP_IF_DIRTY=1` skips instead; unpushed commits always skip), ensures ansible/git exist, parses CLI flags, runs the playbook from `ansible/`. |
| `scripts/deploy.sh` | Legacy: flag-parse + playbook invocation only (no repo sync). Kept for direct use; `~/dev/install.sh` supersedes it. |
