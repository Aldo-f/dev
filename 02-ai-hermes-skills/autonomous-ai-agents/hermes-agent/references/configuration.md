...
### Pitfall: list-typed keys (`custom_providers`)

`hermes config set` cannot navigate into list keys — setting e.g.
`hermes config set custom_providers '[]'` **truncates the whole list** to that
literal. Agent file tools (`patch`/`write_file`) also refuse to write
`~/.hermes/config.yaml` (security-sensitive). To add/modify list-style sections
like `custom_providers`, edit the file directly via a Python `yaml.safe_load` /
`yaml.dump` script (keep a backup first), or have the user edit it by hand.
Scalar keys and `model_aliases` (dict-based, full form with `model`/`provider`/`base_url`)
work fine via `hermes config set`.
