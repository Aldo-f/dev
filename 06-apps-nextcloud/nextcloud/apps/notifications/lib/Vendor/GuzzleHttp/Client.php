...
use OCA\Notifications\Vendor\GuzzleHttp\Cookie\CookieJar;
use OCA\Notifications\Vendor\GuzzleHttp\Cookie\CookieJarInterface;
use OCA\Notifications\Vendor\GuzzleHttp\Exception\GuzzleException;
use OCA\Notifications\Vendor\GuzzleHttp\Exception\InvalidArgumentException;
use OCA\Notifications\Vendor\GuzzleHttp\Handler\CurlShareHandleState;
use OCA\Notifications\Vendor\GuzzleHttp\Handler\CurlVersion;
...
use OCA\Notifications\Vendor\Psr\Http\Message\StreamInterface;
use OCA\Notifications\Vendor\Psr\Http\Message\UriInterface;
/**
 * @final
 */
class Client implements ClientInterface, \OCA\Notifications\Vendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
     ...
     *   OCA\Notifications\Vendor\GuzzleHttp\Promise\PromiseInterface that is fulfilled with a
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        if ($method !== $normalizedMethod) {
            \trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
    ...
    public function request(string $method, $uri = '', array $options = []): ResponseInterface
    {
        $normalizedMethod = Psr7\Utils::asciiToUpper($method);
        if ($method !== $normalizedMethod) {
            \trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = true;
    ...
    private static function normalizeDeprecatedAuthOptionValues(array &$options): void
    {
        ...
        foreach ([0, 1] as $index) {
            if (\array_key_exists($index, $options['auth']) && !\is_string($options['auth'][$index]) && self::canStringifyDeprecatedValue($options['auth'][$index])) {
                $options['auth'][$index] = self::stringifyDeprecatedValue($options['auth'][$index]);
            }
        }
        if (\array_key_exists(2, $options['auth']) && $options['auth'][2] !== null && !\is_string($options['auth'][2]) && self::canStringifyDeprecatedValue($options['auth'][2])) {
    ...
    private static function normalizeDeprecatedTlsFileOptionValues(array &$options, string $option): void
    {
        ...
        foreach ([0, 1] as $index) {
            if (\array_key_exists($index, $options[$option]) && $options[$option][$index] !== null && !\is_string($options[$option][$index]) && self::canStringifyDeprecatedValue($options[$option][$index])) {
    ...
    private static function normalizeDeprecatedStringOptionValues(array &$options): void
    {
        foreach (['cert_type', 'force_ip_resolve', 'ssl_key_type'] as $option) {
            if (\array_key_exists($option, $options) && !\is_string($options[$option]) && self::canStringifyDeprecatedValue($options[$option])) {
    ...
    private static function normalizeDeprecatedNumericOptionValues(array &$options): void
    {
        foreach (['connect_timeout', 'delay', 'read_timeout', 'timeout'] as $option) {
            if (\array_key_exists($option, $options) && \is_string($options[$option]) && \is_numeric($options[$option])) {
    ...
    private static function normalizeDeprecatedIntegerOptionValues(array &$options): void
    {
        foreach (['crypto_method', 'crypto_method_max', 'retries'] as $option) {
            ...
            } elseif (\is_float($options[$option]) && \is_finite($options[$option]) && $options[$option] === (float) (int) $options[$option]) {
                $options[$option] = (int) $options[$option];
            }
        }
    }
    private static function warnAboutRequestLevelHandler(array $options): void
    {
        ...
        \trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    private static function warnAboutInvalidRequestOptionTypes(array $options): void
    {
        ...
        if (isset($options['cookies']) && $options['cookies'] !== false && $options['cookies'] !== true && !$options['cookies'] instanceof CookieJarInterface) {
    ...
    {
        \trigger_deprecation('guzzlehttp/guzzle', $since, 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
